...
SETLOCAL
	...
	REM Attempts to run with JRE
	IF exist "%LaunchPadBatchPath%jclp.bat" CALL "%LaunchPadBatchPath%jclp.bat" :noBrowser
	
    IF "%LaunchPadDefaultBrowser%" == "" GOTO :checkCodePage

    REM Look for html file first
    call :findLocaleFile "noBrowser.html"
...
:checkCodePage
    ...
    call :findCompatCPFile "%LaunchPadBatchPath%%LaunchpadContentDirectory%%NoBrowserLang%\codepages\" "noBrowser.txt" "%CODEPAGE%"
    IF NOT "%RET_findCompatCPFile%" == "" GOTO :showCompatible 
    call :findCompatCPFile "%LaunchPadBatchPath%%LaunchpadContentDirectory%en\codepages\" "noBrowser.txt" "%CODEPAGE%"
    IF NOT "%RET_findCompatCPFile%" == "" GOTO :showCompatible 
    call :findCompatCPFile "%LaunchPadBatchPath%%NoBrowserLang%\codepages\" "noBrowser.txt" "%CODEPAGE%"
    IF NOT "%RET_findCompatCPFile%" == "" GOTO :showCompatible 
    call :findCompatCPFile "%LaunchPadBatchPath%en\codepages\" "noBrowser.txt" "%CODEPAGE%"
    IF NOT "%RET_findCompatCPFile%" == "" GOTO :showCompatible 
