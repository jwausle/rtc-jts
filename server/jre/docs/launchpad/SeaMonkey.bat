...
REM 5648-F10 (C) Copyright International Business Machines Corp. 2006 
...
SETLOCAL
    ...
    CALL "%LaunchPadBatchPath%\readRegistry.bat" "HKEY_LOCAL_MACHINE\Software\Clients\StartMenuInternet\SeaMonkey.exe\shell\open\command" @ moz LaunchPadSeaMonkeyPath

    REM Normalize the path by removing all quotes
    SET LaunchPadSeaMonkeyPath=%LaunchPadSeaMonkeyPath:"=%

    CALL "%LaunchPadBatchPath%\GetShortName.bat" "%LaunchPadSeaMonkeyPath%" LaunchPadBrowser
:gotbrowser
...
:gotURL
    ...
    COPY /Y "%LaunchPadBatchPath%\RunSeaMonkey.bat" "%LaunchPadTemp%\RunSeaMonkey.bat" >nul 2>&1
    "%LaunchPadTemp%\RunSeaMonkey.bat" 2>nul
