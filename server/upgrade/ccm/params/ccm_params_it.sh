...
# # script jts_upgrade.sh. Esso imposta una variabile di ambiente
# # per parametro valido con valore predefinito.
# #
# # È possibile che gli utenti desiderino modificare il valore
# # predefinito per avviare lo script ccm_upgrade.sh con i propri
# # valori senza dover immettere manualmente il parametro ed il
# # relativo valore corrispondente dalla riga comandi.
# #
# # AVVERTENZA:
# # Non tutte le variabili devono essere impostate in base
# # all'applicazione correntemente aggiornata.  Leggere attentamente ogni commento
# # prima di modificare il valore.
# #
# #############################################################
...
#    ad es. /Jazz/IBM/RTC_3.x/jazz/server/conf
#
...
# 	ccm
# Limitazioni:
#  - <root> deve essere un nome singolo di directory valido (senza barre iniziali)
#
...
# Descrizione:
# 	La versione di CCM in fase di aggiornamento.  Per impostazione predefinita, questa è
...
#  - <percorso dir> deve essere una directory esistente che termina con 'server/conf'
#    ad es. /Jazz/IBM/RTC_3.0/JazzTeamServer/server/conf
#
...
# Descrizione:
# 	Il percorso della directory di configurazione del server JTS del JTS che sarà utilizzato dalla nuova installazione
# Valore predefinito:
