...
# #############################################################
# #
# # 定義對應至 jts_upgrade.sh Script
# # 參數的環境變數及其預設值
# #
# #===========================================================
# #
# # 開始執行 jts_upgrade.sh Script 時，會
# # 讀取這個檔案。它會利用預先定義的預設值，
# # 針對每個有效的參數來設定一個環境變數。
# #
# # 使用者可能會想修改預設值，以他們本身
# # 的值來啟動 jts_upgrade.sh Script，而
# # 不必在指令行中手動輸入參數及其對應的
# # 值。                                    
# #
# # 警告：
...
# 說明：
# 	JTS 版已調升。依預設，
#   這是透過對舊 JTS 執行 repotools -version 來判斷。
# 預設值：
# 	<無>
# 限制：
#  - 應為完整的版本字串
