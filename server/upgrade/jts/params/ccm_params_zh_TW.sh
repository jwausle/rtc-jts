...
# #############################################################
# #
# # 定義對應至 ccm_upgrade.sh Script
# # 參數的環境變數及其預設值
# #
# #===========================================================
# #
# # 開始執行 ccm_upgrade.sh Script 時，會
# # 讀取這個檔案。它會利用預先定義的預設值，
# # 針對每個有效的參數來設定一個環境變數。
# #
# # 使用者可能會想修改預設值，以他們本身
# # 的值來啟動 ccm_upgrade.sh Script，而
# # 不必在指令行中手動輸入參數及其對應的
# # 值。                                    
# #
# # 警告：
...
#    例如：/Jazz/IBM/RTC_3.x/jazz/server/conf
#
...
#  - 當接受時，這是必要參數
#  - <dir path> 必須是現有目錄，且結尾是 'server/conf'
#    例如：/Jazz/IBM/RTC_3.0/JazzTeamServer/server/conf
#
...
# 說明：
# 	新安裝架構將使用之 JTS 中 JTS Tomcat 根目錄的路徑。
# 預設值：
# 	新應用程式起始目錄的路徑
# 限制：
#  - <dir path> 必需以 'server/conf' 存在目錄結束
#
NEW_JTS_HOME=
